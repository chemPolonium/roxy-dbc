...
//                 {
//                     if ui_state
//                         .ensure_message_not_in_open_message_windows(message_id)
//                         .is_ok()
//                     {
//                         if let Some(window) = ui_state.dbc_windows.get_mut(idx) {
//                             let mut simple = MessageOverride {
...
//                             if let Some(edit_msg) = window.editable_data.get_message(message_id) {
//                                 simple.message_name = edit_msg.message_name().to_string();
//                                 simple.comment = Some(edit_msg.comment().to_string());
//                                 simple.message_size = edit_msg.message_size();
//                                 simple.transmitter = Some(edit_msg.transmitter().to_string());
//                             } else {
//                                 // Fallback: best-effort defaults
//                                 simple.comment = Some(String::new());
//                                 simple.message_size = 8;
//                                 simple.transmitter = Some(String::new());
//                             }
...
//         // Determine message size in bits (consider overrides if EditableDbc has the message)
//         let msg_size_bytes = dbc_window
//             .editable_data
...
//         // Store a SignalOverride in editable_data via API for both original and custom messages
//         let override_entry = ::SignalOverride {
//             name: sig_name.clone(),
//             start_bit: start_bit,
//             signal_size: size,
...
//         // Prepare old override (if any) from EditableDbc if present
//         let old_override = dbc_window
//             .editable_data
//             .get_message(message_id)
//             .and_then(|m| m.signals().iter().find(|s| s.name() == sig_name))
//             .map(|s| ::SignalOverride {
//                 name: s.name().to_string(),
//                 start_bit: s.start_bit(),
//                 signal_size: s.signal_size(),
//                 byte_order: s.byte_order().clone(),
...
//         if let Some(edit_msg) = dbc_window.editable_data.get_message(message_id) {
//             let new_view = crate::ui::view::MessageView::from_message_ref(
//                 &::MessageRef::CustomOwned(::MessageOverride::from_message(
//                     &can_dbc::Message::default(),
//                 )),
//                 &dbc_window.editable_data,
//             );
//             for sw in ui_state.message_windows.iter_mut() {
...
//             .find(|w| w.id == parent_dbc_id)
//             .map(|window| {
//                 // Use EditableDbc messages() directly
//                 window
//                     .editable_data
//                     .messages()
//                     .iter()
